# input_file = open("20/example.txt"); min_cheat = 50 # 44 
...
cheat_count = dict()
# These are the offsets that if applied to current position, spend between 2 and 20 picoseconds
# It's basically the list of points with manhattan distance between 2 and 20
search_off = {}
for r_off in range(-20,21):
  for c_off in range(-20,21,1):
    dist = abs(r_off) + abs(c_off)
    if( 2 <= dist <= 20 ):
      search_off[(r_off, c_off)] = dist
# print(search_off)
...
for p in points:
  ...
  for off in search_off:
    ...
    if p_off in points:
      ...
      diff = off_points - current_points - search_off[off]
      if diff >= min_cheat:

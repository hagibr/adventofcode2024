...
# Main recursive function to execute the sum or multiplication
def do_math(expected, operands):
  ...
  if count_operands == 2:
    return sum == expected or product == expected
  # Now we will test recursively the 2 operations
  sum_operands = [sum] + operands[2:]
  product_operands = [product] + operands[2:]
  return do_math(expected, sum_operands) or do_math(expected, product_operands)
